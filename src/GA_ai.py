...
class Network:
    def __init__(self, nr_inputs, nr_outputs, nr_hidden_layers, layer_size):
        ...
        if nr_hidden_layers > 1:
            for _ in range(nr_hidden_layers - 1):
                self.hidden_layers = np.append(
                    self.hidden_layers, Layer(layer_size, layer_size)
                )
    ...
    def update_layers(self, code):  # Decoding GA chromosome
        if len(code) != self.total_nr_elements:
            raise RuntimeError(
                f"Code length does not match network architecture: {len(code)} != {self.total_nr_elements}"
            )
        ...
        for layer in self.hidden_layers:
            ...
            weights = np.resize(
                elements[: layer.w_n], (layer.w.shape[0], layer.w.shape[1])
            )
        ...
        weights = np.resize(
            elements[: -self.output_layer.w_n],
            (self.output_layer.w.shape[0], self.output_layer.w.shape[1]),
        )
        biases = np.resize(
            elements[-self.output_layer.b_n :], (self.output_layer.w.shape[0], 1)
        )
...
class Population:
    def __init__(self, population_size: int, code_len: int):
        ...
        self.population = [
            Chromosome(np.random.uniform(low=0, high=0, size=code_len))
