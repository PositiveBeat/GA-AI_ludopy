...
class StateSpace:
    ...
    def canKill(self, die, piece_position):
        if self.willbeKilled(die, piece_position): return False
    ...
    def willbeDanger(self, die, piece_position):
        ...
        if (np.any(np.array(enemy_globes) == position)) : return True
    ...
    def willbeHomeStretch(self, die, piece_position):
        if (piece_position + die >= 53): return True
        if (piece_position + die == player.STAR_AT_GOAL_AREAL_INDX): return True
        else: return False
    
    
    def willbeSafe(self, die, piece_position):
        if (self.willbeHomeStretch(die, piece_position) == True): return True
        position = piece_position + die
...
if __name__ == '__main__':
    ...
    print(state.get_state(2, [0, 0, 7, 30], np.array([[52, 3, 7, 35], [43, 3, 4, 36], [6, 8, 2, 35], [52, 3, 7, 35]])))
